...
//! `target` callback.
//!
//! `setup` also registers the issue #533 surfaces: the
//! `/phasegent-worktree-acquire` command (`context.command.transform`)
//! and the embedded `phasegent-worktree-v2` skill
//! (`context.skill.transform`, same bytes as
//! `skills/phasegent-worktree-v2/SKILL.md`). The adapter targets the
//! OpenCode binary's runtime plugin context, not the npm
//! `@opencode-ai/plugin` type package, which can lag the binary (1.18.25
//! exposes no `tool`, `worktree`, `session`, or `location`); a missing
//! registration surface degrades to a console warning.
