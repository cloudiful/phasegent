...
pub(crate) fn print_admin_config_command_help(role: Option<Role>, command: &str) {
    match command {
        "set" => {
            let role_text = role.map_or("ROLE", Role::as_str);
            println!(
                "Usage: PHASEGENT_ROLE={role_text} phasegent admin config set <SETTING> [VALUE|--stdin]\n\nHuman-operator only; AI roles must never invoke the admin group.
...
Non-secret settings use a positional value or --stdin. Index selection is URL-driven (no index-backend value needed):
  PHASEGENT_ROLE=executor phasegent admin config set api-base https://forgejo.example
  PHASEGENT_ROLE=executor phasegent admin config set api-base --stdin
