...
pub(crate) fn print_config_command_help(role: Option<Role>, command: &str) {
    match command {
        ...
        "clear" => {
            println!(
                "Usage: phasegent [--role ROLE] config clear <SETTING>\n\nRemoves the persisted setting from SQLite. Prints the canonical setting name and whether a row/field was cleared.\n\nGlobal settings are machine-wide and can be cleared without --role. Role-scoped settings require --role.\nThe bearer key (`redmine-git-mirror-api-key`) and index PG URL are reported only as presence/length in `config show`. Clearing `index-pg-url` returns the index to SQLite; clearing legacy `index-backend` never changes selection. `config clear` removes SQLite only; a TOML phasegent.toml value still shadows until the file (or env) is removed."
            );
        }
