...
pub(crate) fn print_timer_help(role: Option<Role>) {
    if role.is_none_or(|role| role == Role::Orchestrator) {
        println!(
            "Timer commands for orchestrators (internal/auto-managed; CLI is manual fallback and recovery only — AI workflows should not call these directly):\n\n  start <ISSUE> --phase NAME --agent-role executor|reviewer|tester --attempt N [--run-id ID] [--owner-session-id S --owner-call-id C]    Persist a local phase run (manual fallback; the lifecycle path opens runs automatically on status transitions)\n  finish <RUN_ID> --result DONE|PARTIAL|BLOCKED|FAILED    Finish a manually-opened run and project its rounded time to Redmine or GitLab (auto-finish happens on the next status transition)\n  list [--status running|finished|all] [--limit N]        Inspect local phase runs (read-only, local-only)\n  get <RUN_ID>                                            Show one local phase run (read-only, local-only)\n  recover <RUN_ID>                                         Mark a known orphan FAILED and project via the configured provider\n\nTimer ledger writes are local-only and must happen before any projection. Finish and recover are Redmine- or GitLab-only (Forgejo rejects both) and orchestrator-only. List and get never make provider or network requests; they expose the SQLite ledger minus secrets and full responses. Recover is the explicit orphan path: it never infers success, never reopens a finished row, and reuses the same-run provider reconciliation as finish. tester is a first-class role with its own Redmine credential (IssueRead, CommentRead/FindMarker/Create with --authorized, and IssueAttachmentUpload reserved for future re-enable); timers remain orchestrator-only and --agent-role tester is persisted as a child run identity with optional bootstrap membership."
        );
    } else {
        println!(
