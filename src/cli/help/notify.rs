...
pub(crate) fn print_notify_command_help(role: Option<Role>, command: &str) {
    match command {
        "send" => {
            ...
            println!(
                "Usage: PHASEGENT_ROLE={role_text} phasegent notify send --event <EVENT> --title <TITLE> [--body <BODY>] [--issue <ID>] [--phase <PHASE>]\n\nDelivers one bounded notification envelope on the configured channel (ntfy, webhook, dingtalk with notify-dingtalk, email with notify-email).\n\n  --event   completion | blocked | failure | interruption_suspected | publish_failed\n  --title   short summary (required, truncated to 140 chars)\n  --body    bounded detail (optional, truncated to 2000 chars)\n  --issue   optional positive issue id added to body/metadata\n  --phase   optional phase label stored as metadata\n\nConfigure with `admin config set notify-enabled true`, `admin config set notify-channel <name>`, plus per-channel fields (ntfy-base-url, ntfy-topic, webhook-url, ...). Secrets (ntfy-token, webhook-token, dingtalk-secret, email-password) require --stdin. The admin group is human-operator only. No automatic triggers; explicit send only. Manual send returns a structured notification error when delivery fails."
            );
        }
