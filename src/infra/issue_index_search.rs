...
pub fn lexical_search_inner(
    ...
    let rows = stmt
        .query_map(
            params![escaped_query, limit as i64, offset as i64],
            |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, String>(1)?,
...
mod tests {
    ...
    fn fts_term_unicode_and_invalid_query_behavior() {
        ...
        let doc1 = IssueIndexDocument::new(key1, 1, "hello world".into(), "body with 汉字 😀".into(), "open".into(), None, None, 1_700_000_000).unwrap();
        idx.upsert(&doc1).unwrap();
        // ASCII term
        let res = idx.lexical_search("hello", 10, 0, false).unwrap();
        ...
        let res = idx.lexical_search("汉字", 10, 0, true).unwrap();
        ...
        let res = idx.lexical_search("😀", 10, 0, false).unwrap();
        assert!(res.total_count <= 1);
        // Invalid/empty query must be config error, not panic
        assert!(idx.lexical_search("   ", 10, 0, false).is_err());
        assert!(idx.lexical_search("", 10, 0, false).is_err());
        // Malformed FTS syntax should be escaped, not crash: e.g., `OR` or `"`.
        let _ = idx.lexical_search("OR", 10, 0, false).unwrap();
        let _ = idx.lexical_search("\"hello", 10, 0, false).unwrap();
    ...
    fn output_compactness_and_truncation() {
        ...
        let doc = IssueIndexDocument::new(key, 2, "long".into(), long_body.clone(), "open".into(), None, None, 1_700_000_001).unwrap();
        idx.upsert(&doc).unwrap();
        // Without include_body, body omitted and not truncated
        let res = idx.lexical_search("long", 10, 0, false).unwrap();
        assert!(res.items[0].body.is_none());
        assert!(res.items[0].body_truncated.is_none());
        // With include_body, body capped to 8192 and marked truncated
        let res = idx.lexical_search("long", 10, 0, true).unwrap();
        assert_eq!(res.items[0].body_truncated, Some(true));
        assert_eq!(res.items[0].body.as_ref().unwrap().len(), crate::providers::api::ISSUE_SEARCH_MAX_BODY_BYTES);
        assert!(res.items[0].body.as_ref().unwrap().is_char_boundary(res.items[0].body.as_ref().unwrap().len()));
        // Short body not truncated
        let key2 = IssueIndexKey::new("forgejo", "owner/repo", "3").unwrap();
        let doc2 = IssueIndexDocument::new(key2, 3, "short".into(), "short".into(), "open".into(), None, None, 1_700_000_002).unwrap();
        idx.upsert(&doc2).unwrap();
        let res = idx.lexical_search("short", 10, 0, true).unwrap();
    ...
    fn fts_round_trip_after_upsert_and_resurrection() {
        ...
        let doc = IssueIndexDocument::new(key.clone(), 99, "alpha".into(), "beta body".into(), "open".into(), None, None, 1_700_000_100).unwrap();
        idx.upsert(&doc).unwrap();
        assert_eq!(idx.lexical_search("alpha", 10, 0, false).unwrap().total_count, 1);
        // Tombstone removes from FTS
        idx.tombstone(&key, 1_700_000_200).unwrap();
        assert_eq!(idx.lexical_search("alpha", 10, 0, false).unwrap().total_count, 0);
        assert_eq!(idx.lexical_search("beta", 10, 0, false).unwrap().total_count, 0);
        // Resurrection brings back
        let doc2 = IssueIndexDocument::new(key.clone(), 99, "alpha".into(), "beta body revived".into(), "open".into(), None, None, 1_700_000_300).unwrap();
        idx.upsert(&doc2).unwrap();
        assert_eq!(idx.lexical_search("alpha", 10, 0, false).unwrap().total_count, 1);
        assert_eq!(idx.lexical_search("revived", 10, 0, false).unwrap().total_count, 1);
        ...
        idx.tombstone(&missing, 1_700_000_400).unwrap();
        assert_eq!(idx.lexical_search("deleted", 10, 0, false).unwrap().total_count, 0);
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn stable_deterministic_ordering_for_ties() {
        ...
        let d1 = IssueIndexDocument::new(k1, 10, "same".into(), "same body".into(), "open".into(), None, None, 1_700_000_000).unwrap();
        let d2 = IssueIndexDocument::new(k2, 2, "same".into(), "same body".into(), "open".into(), None, None, 1_700_000_001).unwrap();
        idx.upsert(&d1).unwrap();
        idx.upsert(&d2).unwrap();
        let res = idx.lexical_search("same", 10, 0, false).unwrap();
        ...
        let res2 = idx.lexical_search("same", 10, 0, false).unwrap();
    ...
    fn lexical_search_is_local_only_and_no_network_needed() {
        ...
        let _guard = crate::infra::storage::test_support::EnvGuard::set("PHASEGENT_INDEX_DB_PATH", path.to_str().unwrap());
        let idx = SqliteIssueIndex::open().unwrap();
        let key = IssueIndexKey::new("gitlab", "42", "1").unwrap();
        let doc = IssueIndexDocument::new(key, 1, "local".into(), "no network needed".into(), "open".into(), None, None, 1_700_000_000).unwrap();
        idx.upsert(&doc).unwrap();
        let res = idx.lexical_search("local", 10, 0, false).unwrap();
        assert_eq!(res.total_count, 1);
